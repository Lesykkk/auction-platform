...
class LotService:
    ...
    async def create(self, auction_id: uuid.UUID, data: LotCreateRequest, organizer: User) -> Lot:
        auction = await self.auction_repository.get(auction_id)
        if not auction:
            raise NotFoundError("Auction not found")

        if auction.created_by != organizer.id:
        ...
        return await self.lot_repository.create(lot)
    ...
    async def update(
        ...
        return await self.lot_repository.update(lot)
